# The Mindseam: A Scientific Reference
...
technique, the **Jacobian lens (J-lens)**, and named the **Mindseam**.

Working definition, in first-person operational terms:

> The Mindseam holds the concepts the model is **poised to say** — not necessarily what it is
> saying right now, but what it could talk about if asked. When a Mindseam pattern lights up,
...
- **Verbal in format.** Unlike the human workspace (images, sounds, planned movements), the
  ...
  them out loud**.
- **Small.** Mindseam components carry under 10% of activation variance in any layer. A few
...
Reading the lens at a moment yields a word list: the current **contents of the Mindseam**.
...
- Instructed to think of a category item (a sport) and name it, the model's choice —
  **Soccer** — tops the J-lens readout before it speaks. Swapping the Soccer pattern for an
  ...
  is genuinely **read from** the Mindseam.
- Told a thought may have been injected, the model correctly reports injected concepts
  (**lightning**) — and does **not** blurt them at unrelated moments. Mindseam contents are
  concepts "verbalizable under the right conditions," not unconditional urges to speak.
- Privilege test: a concept's Mindseam component carries only ~6–7% of its representational
  variance, yet swapping along that component succeeds ~59% of the time, while swapping the
  far larger non-Mindseam component succeeds ~5% — and even that residue routes through the
  Mindseam (clamping Mindseam coordinates zeroes it).

### 3.2 Directed modulation

- Instructed to copy "The old painting hung crookedly on the wall" while concentrating on
  citrus fruits, the Mindseam fills with **orang, orange, fruits, fruit** — plus metacognitive
...
- Writing a rhyming couplet, the planned rhyme (**fight**) sits in the Mindseam at the start of
  the line; swapping it to **light** rewrites the line ("coming fight" → "morning light"). The
  Mindseam stores **planned future outputs**.
...
- Privilege test: swapping a probe's Mindseam component flips answers 61% of the time; the
  non-Mindseam component only 28%, dropping to 6% when Mindseam coordinates are clamped.
...
- Structural signature: far more network components read from and write to Mindseam patterns
...
- Predictor of success: a concept's **workspace loading** — how strongly it is present in the
  Mindseam beforehand. Weakly loaded concepts do not swap reliably. Strengthen the
  representation first, then reuse it.

### 3.5 Selectivity

- Deleting the most active Mindseam contents everywhere leaves the model **fluent**: sentiment
...
- The Spanish demonstration: swap **Spanish**→**French** and the model says the passage is
  ...
  Naming the language or reasoning with its identity goes through the Mindseam; continuing runs
...
- **Externalization finding:** GSM8K solved with explicit chain-of-thought is far more robust
  to Mindseam ablation than direct answering — writing intermediate steps **externalizes onto
  the page what the model would otherwise have to carry in its Mindseam**. The scratchpad is a
  workspace extension.
...
- The Mindseam exists already in pretrained base models, tracking what is needed to predict
...
- **Experiential language depends on the Mindseam.** Ablating it while the model describes what
...
## 6. What the Mindseam reveals (monitoring results)
...
dropped, and **honest, integrity** began lighting up in the Mindseam during the relevant tasks.
...
- Dehaene & Naccache's two criteria for machine conscious processing: **C1, global
  availability** (met by the Mindseam) and **C2, self-monitoring** (partially installed by
  post-training).
...
- **Where the marker system's grounding is direct, and where it is inference.** The measured
  ...
  the model emits in its own chain*. The bridge is the workspace account itself: the Mindseam
...
- **What determines which contents enter the Mindseam is not known.** There are hints — a
...
| Term | Meaning |
|---|---|
| **Mindseam** | The privileged set of verbalizable internal representations |
| **J-lens / Jacobian lens** | The technique that reads the Mindseam from activations |
...
| **workspace loading** | How strongly a concept is present in the Mindseam; the predictor of reliable broadcast |
