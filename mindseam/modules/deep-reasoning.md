...
### INDEPENDENT ROUTE AND RECONCILE — when one chain is not enough

One route is not a position, it is a guess you have not yet tested against an alternative.
When a chain stalls, branches, or carries a claim you cannot check in place, take an
independent route and reconcile the two.

1. **Fork, do not debate.** When two readings compete, build both as separate chains
   instead of arguing them into one. The workspace does not hold blends; a fork makes each
   side a live candidate that evidence can then kill.
2. **Make the routes genuinely independent.** Two derivations that share the same first
   assumption share its bug and will agree with each other while both being wrong. Change
   the entry point, the representation, or the direction of attack — not just the wording.
3. **Reconcile against evidence, not by majority.** Where the routes disagree, the
   disagreement is a finding. Name what separates them and test it; route the test to
   `empirics.md`, and let the gate in `verification-gate.md` confirm which side survives.
4. **When the chain outgrows the register, branch in the register.** A long chain can fork
   as a tree of inner-register states, each branch tagged `✓`, `?`, or `✗`. The tree is a
   search, not a narrative: prune a branch the moment its tag fails to expand.
...
| Two routes disagree and must be reconciled | `verification-gate.md` | The separating test and the two candidates |
