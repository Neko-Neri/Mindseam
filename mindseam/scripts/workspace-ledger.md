# Mindseam Workspace Ledger — template

Copy the blank shell below to `.mindseam/WORKSPACE.md` at the start of any task that will span
stages, files, or turns, then fill `Goal` and `Next` before the first seam. Or let
`mindseam.py note --goal "..." --next "..."` create it. If there is no filesystem here, keep the
...
`.mindseam/` is working state, not part of the project. It belongs in `.gitignore`, and it should
not be committed unless someone asks for it:

```
echo ".mindseam/" >> .gitignore
```
