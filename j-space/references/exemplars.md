...
Everything here is quoted from observed records — J-lens readouts from the workspace research,
and raw reasoning traces from frontier models. None of it is invented for illustration, which
is the point. These are not what the workspace *might* look like. They are what it does look
like.
...
| `window ∋ i ✓ retro ✓ consistent: used[i] starts 0 ✓ ✓` | Window contains *i* — checked. Retroactive counting — checked. Consistency given a zero start — checked. |
