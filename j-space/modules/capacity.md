...
> I am reading this because more is live than fits. I choose what gets the stage, and
> everything else goes on the page.
...
If you cannot say which test would separate them, it was never a candidate set.
...
3. **Append, never rewrite.** Verified entries are numbered so a rollback has an address:
   "return to the last verified checkpoint" is only executable if the checkpoints have
   numbers.
...
| A checkpoint is ready to record | `markers.md` | The record and its number |
