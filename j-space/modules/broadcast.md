...
> I am reading this because several parts of this task need the same thing. I write it once,
> strongly, and every part reads that one copy.
...
3. **Load each one.** Loading is a three-step operation and all three are required:
   ...
   sub-task.
4. **Declare the set:** *"For everything that follows, the core is `<a>`, `<b>`, `<c>`."*
   Two live entries is the working limit; more than that belongs in the ledger.
...
| A part completed and verified | `markers.md` | The one-line record |
