...
<python-command> <skill-root>/scripts/jspace.py seam --from-stdin                   # read one next action per line from standard input (like kubectl apply -f - / xargs)
...
<python-command> <skill-root>/scripts/jspace.py info --list-fields                  # describe the ledger schema (like kubectl explain / man page)
