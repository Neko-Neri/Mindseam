...
turns. Or let `jspace.py note` create it. Or, if there is no filesystem here, keep these five
lines in the conversation and restate them at each seam — the file was never the point, the
re-reading was.
...
2. **Append, never rewrite.** Verified entries are numbered so that a rollback has an address.
   "Return to the last verified checkpoint" is only executable if the checkpoints have numbers.
