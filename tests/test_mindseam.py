...
LEDGER_TEMPLATE = ROOT / "mindseam" / "scripts" / "workspace-ledger.md"

from _controller_helper import _clear_mindseam, run_controller, _run_verify_suite
...
class JSpaceControllerTests(unittest.TestCase):
    ...
    def setUp(self):
        _clear_mindseam(self.workspace)
    ...
    def run_controller(self, *args, stdin=None):
        return run_controller(self.workspace, *args, stdin=stdin)

    def run_controller_bytes(self, *args, stdin=None):
        from _controller_helper import _RunControllerResult
        return _RunControllerResult.call_bytes(self.workspace, *args, stdin=stdin)
    ...
    def test_history_recovery_and_register_inspection(self):
        ...
        seam = self.run_controller("seam")
        self.assertEqual(seam.returncode, 0, seam.stdout + seam.stderr)
        self.assertIn("history root was not a list and has been restarted", seam.stdout)
    ...
    def test_integrity_checker_rejects_controller_anchor_drift(self):
        copied_skill = Path(self.workspace) / "mindseam"
        shutil.copytree(ROOT / "mindseam", copied_skill)
        copied_controller = copied_skill / "scripts" / "mindseam.py"
        ...
        checked = _run_verify_suite(copied_skill)
        self.assertEqual(checked[0], 1, checked[1] + checked[2])
        self.assertIn("PREMISE differs from SKILL.md", checked[1])
        self.assertIn("INVARIANTS differ from SKILL.md", checked[1])
    ...
    def test_integrity_checker_requires_exact_entry_routes(self):
        ...
        checked = _run_verify_suite(copied_skill)
        self.assertEqual(checked[0], 1, checked[1] + checked[2])
        self.assertIn("modules{}introspection.md: not routed".format(os.sep), checked[1])
